	...
	public class MatchMediator : DataAccess, IMatchMediator
	{
        ...
        public void Add(Match match,int tournamentID)
        {
            ...
            if (ConnOpen())
            {
                try
                {
                    query = "INSERT INTO match_syn(player1,player2,resultOfPlayer1,resultOfPlayer2,tournament)" +
                   "VALUE (@player1,@player2,@resultOfPlayer1,@resultOfPlayer2,@tournament)";
                    SqlQuery(query);
            ...
            try
            {
                ...
                if (ConnOpen())
                {
                    ...
                    while (dataReader.Read())
                    {
                        ...
                        Match match = new Match(user1, user2, Convert.ToInt32(dataReader["resultOfPlayer1"]), Convert.ToInt32(dataReader["resultOfPlayer2"]));
        ...
        public bool Update(Match match)
        {
            if (ConnOpen())
            {
                try
                {
                    query = "UPDATE match_syn SET resultOfPlayer1 = @resultOfPlayer1 ,resultOfPlayer2 = @resultOfPlayer2 WHERE id = @id";
                    SqlQuery(query);
                    AddWithValue("@resultOfPlayer1", match.ResultPlayer1);
                    AddWithValue("@resultOfPlayer2", match.ResultPlayer2);
